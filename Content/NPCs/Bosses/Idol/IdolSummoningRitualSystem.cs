...
public partial class IdolSummoningRitualSystem : ModSystem
{
    ...
    private void UpdateSounds()
    {
        ...
        if ((BaseWindSoundInstance is null || BaseWindSoundInstance.HasBeenStopped) && BaseWindSoundVolume > cutoffVolume)
            BaseWindSoundInstance = LoopedSoundManager.CreateNew(new SoundStyle("IdolOfMadderCrimson/Assets/Sounds/Environment/IdolSummonWind"), () => BaseWindSoundVolume <= cutoffVolume);
        if ((HarshWindSoundInstance is null || HarshWindSoundInstance.HasBeenStopped) && HarshWindSoundVolume > cutoffVolume)
            HarshWindSoundInstance = LoopedSoundManager.CreateNew(new SoundStyle("IdolOfMadderCrimson/Assets/Sounds/Environment/IdolSummonWindHarsh"), () => HarshWindSoundVolume <= cutoffVolume);
    ...
    private static void UpdateLoopSound(LoopedSoundInstance? sound, float volume)
    {
        ...
        if (sound.HasLoopSoundBeenStarted && wasInactive && SoundEngine.TryGetActiveSound(sound.LoopingSoundSlot, out ActiveSound? soundInstance) && soundInstance is not null && soundInstance.Sound is not null)

    ...
    private string UseWeatherText(string originalText)
    {
        if (WasInSubworldLastFrame)
        {
            if (MathF.Abs(Main.windSpeedCurrent) >= 0.03f)
                return Language.GetTextValue("Mods.IdolOfMadderCrimson.CellPhoneInfoOverrides.ForgottenShrineWindyText");

            return Language.GetTextValue("Mods.IdolOfMadderCrimson.CellPhoneInfoOverrides.ForgottenShrineText");
        }
